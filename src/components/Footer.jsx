            ...
            <div className="flex gap-3">
              {brCfg.socials.map(({ Icon, url }, i) => (
                <a 
                  key={i} 
                  href={url} 
                  className={`w-10 h-10 rounded-full border border-gray-100 flex items-center justify-center text-${brCfg.colors.primary} hover:bg-${brCfg.colors.secondary} hover:text-white hover:border-${brCfg.colors.secondary} transition-all duration-500 shadow-sm`}
                >
                  <Icon size={14} />
                </a>
              ))}
